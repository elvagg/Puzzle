...
def process(set_dir, image_count, plot=False, write_spec=False, cc=1):
    ...
    for i in range(image_count):
        ...
        ip = ImageProcessor()
        ip.read_img(path)
        curved_edges.append(ip.process_image())
        if plot:
            ip.plot()
    nn = NearestNeighbors(n_neighbors=5, metric=dist_metric).fit(curved_edges)
    images_ranking = nn.kneighbors(curved_edges, return_distance=False)
    ranks = []
    for x in images_ranking:
        rank = ""
        for y in x:
            rank += str(y) + " "
        ranks.append(rank)
